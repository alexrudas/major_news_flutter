    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: ListTile(
          ...
          subtitle: Text(controluser.userActive!.userName),
        ),
        actions: [
          IconButton(
              onPressed: () {
...
class VistaLocations extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < locations.length; i++) {
      var location = locations[i] as DocumentSnapshot<Map<String, dynamic>>;
      if (location.data()!.keys.length > 1) {
        if (uid != locations[i]['uid']) {
          print(locations[i]['lat'].runtimeType);
